...
void xyq::xhttp_server::add_path(std::string path, xyq::xhttp_rsp_func rsp_function) {
    ...
    */
    ...
    std::cout << "ADD PATH MAPPING: " << path << std::endl;
}
...
void xyq::xserver_base::show_info() const {
    char buffer[128];
    snprintf(buffer, sizeof(buffer), "%s/%s:%d %d %d", this->__protocal.data(), this->__ip.data(), this->__port,
             this->__status, this->__socket);
...
xyq::xhttp_connect::xhttp_connect(int socket, const struct sockaddr_in &addr, xhttp_server *server) : xconnect_base(
        socket, addr) {
...
xyq::xhttp_connect::xhttp_connect(int socket, const struct sockaddr_in &addr, xhttp_server *server, uint64_t id)
        : xconnect_base(socket, addr) {
    this->start_time_point = std::chrono::system_clock::now();
    if (server != NULL) {
        this->__server = server;
        this->set_cid(id);
    ...
    */
    try {
        ...
        if (not clnt_req.enable) {
            ...
            std::cout << xyq::get_time_now() << "<" << this->__cid << ">"
                      << "[" << clnt_req.get_ip() << "]: " << clnt_req.get_method() << " " << clnt_req.get_path()
                      << std::endl;
            ...
            if (rsp_func) {
                ...
                std::cout << "NOT FOUND PATH" << clnt_req.get_path() << std::endl;
                clnt_rsp.not_found();
                this->update_status(xyq::xconnect_status::FINISH);
            }
